...
Очередь кончилась — бот продолжает сам: берёт радио-микс последнего трека и подбирает следующие. Новую пачку он подтягивает заранее, пока играет последний трек, поэтому паузы между треками нет. Заказы всегда важнее радио: кинул ссылку — она заиграет следующей, а подобранное радио уступит место и вернётся, когда заказы кончатся. Остался один в канале — через 3 минуты замолкает. Написал из другого канала — бот придёт к тебе сам (в запароленный — с тем паролем, что помнит).
...
| `Update yt-dlp.cmd` | обновить yt-dlp руками; обычно не нужно — бот делает это сам |
...
YouTube время от времени меняет способ подписи ссылок, и устаревший yt-dlp перестаёт их открывать. Бот узнаёт такую ошибку по ответу yt-dlp (она отличается от «видео удалено»), сам обновляет его и повторяет запрос — руками ничего запускать не надо; плюс раз в сутки проверяет обновление на всякий случай.
